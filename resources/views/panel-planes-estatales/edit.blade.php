<x-app-layout>
    @section('title', 'Editar Plan Estatal')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Plan Estatal') }}
        </h2>
    </x-slot>

    <div class="contenedor-principal">
        <div class="encabezado-lista my-2">
            <h2>Editar Plan Estatal</h2>
        </div>
        <div class="container">
            <form action="{{ route('panel-cat-planes.update', $plan->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('gobernador') is-invalid @enderror"
                        name="gobernador" id="gobernador" value="{{ old('gobernador', $plan->gobernador) }}" required>
                    @error('gobernador')
                        <small class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </small>
