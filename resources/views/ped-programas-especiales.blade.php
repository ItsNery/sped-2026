...
@section('twitter-description',
    ...
    Estado de Puebla')
@section('content')
    <img src="" alt="Banner de los Programas Derivados Especiales" title="Banner de los Programas Derivados Especiales"
        width="100%" class="px-0">
            ...
            @foreach ($especiales as $especial)
                <div class="col-6 col-sm-3 hvr-grow my-3">
                    <a href="{{ url('/ped-programas/especiales/' . Illuminate\Support\Str::slug($especial->nombre)) }}"
                        ...
                        <img class="img-fluid" src="{{ asset($especial->imagen) }}" title="{{ $especial->nombre }}"
                            alt="{{ $especial->nombre }}" />
                    </a>
                </div>
            @endforeach
