<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Gestiona tus sesiones activas en otros navegadores y dispositivos.') }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div>
            {{ __('Si fuera necesario, puedes cerrar sesión en todos los demás navegadores en tus diferentes dispositivos. Algunas de tus recientes sesiones están listadas abajo; sin embargo, esta lista puede no ser exhaustiva. Si sientes que tu cuenta ha sido comprometida, deberias actualizar también tu contraseña.') }}
        </div>
        ...
        <div class="d-flex mt-3">
            <x-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Cerrar sesión en otros dipositivos') }}
            </x-button>
        </div>
        ...
        <x-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Por favor ingresa tu contraseña para confirmar que quieres cerrar sesión en otros navegadores en todos tus dispositivos.') }}

                <div class="mt-3 w-md-75" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-input type="password" placeholder="{{ __('Contraseña') }}"
                                 x-ref="password"
